...
3.  Aggregate the samples $T_{(j)}$ for each region $A_i$ to obtain samples of the areal-level predictive target $\mathcal{M}_i$. Denote these samples as $\mathcal{M}_i^{(j)}$. 
